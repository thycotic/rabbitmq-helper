...
Scenario: BasicPublish calls Publish on ExchangeDictionary
	When the method BasicPublish on MemoryMqWcfServer MemoryMqWcfServerTest is called with exchange ExchangeTest and routing key RoutingKeyTest
...
Scenario: QueueBind calls AddBinding on BindingDictionary
	When the method QueueBind on MemoryMqWcfServer MemoryMqWcfServerTest is called
...
Scenario: BasicConsume calls AddClient on ClientDictionary
	When the method BasicConsume on MemoryMqWcfServer MemoryMqWcfServerTest is called
...
Scenario: BasicAck calls Acknowledge on ExchangeDictionary
	When the method BasicAck on MemoryMqWcfServer MemoryMqWcfServerTest is called
...
Scenario: BasicNack calls NegativelyAcknowledge on ExchangeDictionary
	When the method BasicNack on MemoryMqWcfServer MemoryMqWcfServerTest is called
...
Scenario: Dispose calls Stop on MessageDispatcher
	When the method Dispose on MemoryMqWcfServer MemoryMqWcfServerTest is called
