...
REM %ig% generateGenericInstallerRunnerZip -ArtifactName=girapp.zip -ArtifactPath=%artifactPath% -SourcePath.Binaries=-SourcePath.Binaries=%illidanMapPath%\src\Thycotic.InstallerGenerator\Thycotic.InstallerRunner\bin\Release -Installer.Version=5.0.0.0 -Signing.PfxPath=%pfxPath% -Signing.PfxPassword=%pfxPassword%

echo NO SIGNING PAST THIS POINT

echo Generating configured installers/updaters...
echo 64Bit Memory Mq site connector
%ig% generateConfiguredMemoryMqSiteConnectorServiceZip -ArtifactName=cmmqsvc.zip  -ArtifactPath=%artifactPath% -SourcePath.MSI=%artifactPath%\gmmqsvc.msi -RunnerZipPath=%artifactPath%\girapp.zip -Installer.Version=5.0.0.0 -Pipeline.ConnectionString=net.tcp://illidan:8671 -Pipeline.UseSsl=false -Pipeline.Thumbprint=invalid